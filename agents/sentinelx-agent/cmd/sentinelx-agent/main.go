...
func post(c config,path string, body any, token string, out any) error { b,_:=json.Marshal(body); var last error; for attempt:=0;attempt<3;attempt++ { req,e:=http.NewRequest(http.MethodPost,c.APIURL+path,bytes.NewReader(b));if e!=nil{return e};req.Header.Set("Content-Type","application/json");if token!=""{req.Header.Set("Authorization","Agent "+token)};res,e:=(&http.Client{Timeout:15*time.Second}).Do(req);if e==nil { data,_:=io.ReadAll(res.Body);res.Body.Close();if res.StatusCode>=200&&res.StatusCode<300 {if out!=nil{return json.Unmarshal(data,out)};return nil};last=fmt.Errorf("agent API %s: %s",res.Status,string(data)) } else {last=e};if attempt<2 {time.Sleep(time.Duration(1<<attempt)*500*time.Millisecond)} };return last }
