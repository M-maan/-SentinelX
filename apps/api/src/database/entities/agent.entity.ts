...
export class Agent {
  ...
  @Index() @Column({ length: 255 }) hostname!: string;
  @Index() @Column({ name: 'operating_system', length: 80 }) operatingSystem!: string;
  ...
  @Index() @Column({ name: 'agent_version', length: 80 }) agentVersion!: string;
  ...
  @Index() @Column({ name: 'last_seen', type: 'timestamptz', nullable: true }) lastSeen!: Date | null;
