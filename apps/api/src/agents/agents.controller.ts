...
@Controller({ path: 'agents', version: '1' }) export class AgentsController {
  ...
  @Get('dashboard/summary') @UseGuards(JwtAuthGuard, RolesGuard) @Roles(Role.SUPER_ADMIN, Role.SECURITY_ADMIN, Role.ANALYST, Role.VIEWER) summary(@Req() req: UserRequest) { return this.agents.summary(req.user); }
  @Get() @UseGuards(JwtAuthGuard, RolesGuard) @Roles(Role.SUPER_ADMIN, Role.SECURITY_ADMIN, Role.ANALYST, Role.VIEWER) list(@Req() req: UserRequest, @Query('page') page?: string, @Query('limit') limit?: string, @Query('status') status?: AgentStatus, @Query('os') operatingSystem?: string, @Query('search') search?: string, @Query('agentVersion') agentVersion?: string, @Query('sort') sort?: string) { return this.agents.list(req.user, Math.max(1, Number(page ?? 1)), Math.min(100, Math.max(1, Number(limit ?? 25))), status, operatingSystem, search, agentVersion, sort); }
  @Get(':id/telemetry') @UseGuards(JwtAuthGuard, RolesGuard) @Roles(Role.SUPER_ADMIN, Role.SECURITY_ADMIN, Role.ANALYST, Role.VIEWER) agentTelemetry(@Req() req: UserRequest, @Param('id') id: string, @Query('limit') limit?: string) { return this.agents.getTelemetry(req.user, id, Math.min(100, Math.max(1, Number(limit ?? 30)))); }
